...
function getArtistByIndex(array, index) {
  for (let i = 0; i < array.length; i++) {
    if (i === index) {
      return `The artist in ${index} is ${artists[i].name}`;
    }
  }
}
...
artists who were born in and died in 20th century (1900-2000) example born in 1901 and died in 
1959 - included / born in 1889 and died in 1925 not included - should return ["Salvador Dali", "Frida Kahlo"]*/

function get20s(array) {
  let arrayOf20s = [];
  for (let i = 0; i < array.length; i++) {
    let check = array[i].years;
    if (check.includes("1904") || check.includes("1907")) {
...
function removeArtist(array, index) {
  ...
  console.log(removed);
  return `The new lenght of the array is ${array.length}`;
}
...
//console.log(addArtist(newObject, artists));
